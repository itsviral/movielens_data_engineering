...
class TopMoviesPerUser:
    ...
    def get_top_3_movies(self):
        try:
            ...
            ranked_df = self.ratings_df.withColumn("rank", row_number().over(window_spec))
            top_3_movies_df = ranked_df.filter(col("rank") <= 3)
            top_3_movies_with_titles_df = top_3_movies_df.join(self.movies_df, "MovieID", "left").select("UserID", "Title", "Rating")
